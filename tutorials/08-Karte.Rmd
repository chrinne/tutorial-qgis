...
**[Ausdruck einfügen/bearbeiten...]** oder **[Dynamischer Text]** bieten die Möglichkeit, auf Eigenschaften von Objekten und Variablen zuzugreifen, z.B. den Maßstab eines Kartenfensters. Dieser Text wird dann automatisch angepasst. Fügen Sie am unteren Rand ein neues Textfeld ein ("Beschriftung hinzufügen") und wählen sie im Bereich der Elementeigenschaften [Dynamischer Text] > Karteneigenschaften > Karte 1 > Maßstab. Im Editierfenster steht nun ```[%item_variables('Karte 1')['map_scale']%]``` und im Textfeld der Maßstab, allerdings mit vielen Nachkommastellen. Nehem Sie die Syntax aber als Vorlage und verändern Sie nachfolgend den bereits eingefügten Text (s.o.).

Gehen Sie in den eingefügten Text, fügen Sie vor "Karte: ..." eine Leerzeile ein und wählen Sie nun [Ausdruck einfügen...]. In dem sich öffnenden Ausdruckseditor wählen Sie im mittleren Bereich **Maps > map_get** und lesen in der zugehörigen Hilfe das Beispiel "map_get( item_variables('Map 0'), 'map_scale') → Maßstab des Kartenelements 'Map 0' (wenn es vorhanden ist) im aktuellen Drucklayout". Das ist genau das, was wir wollen. Fügen Sie entsprechend links folgenden Ausdruck ein: 

```concat('M 1:',round(map_get( item_variables('Karte 1'), 'map_scale')),0)```

Die Vorschau sollte nun "M 1:1000" oder ggf. einen anderen Maßstab anzeigen. Von Innen nach Außen sind es folgende Schritte: 1. Die Abfrage des Maßstabes nach der Hilfe, allerdings angepasst auf 'Karte 1'. 2. Die Rückgabe wird gerundet auf 0 Stellen: round(\<Wert>,\<Stellen>). 3. Mit 'M 1:' zusammengesetzt: concat(\<Text1>, \<Text2>, ...). Mit [OK] wird der Ausdruck übernommen und die "[%...%]" ergänzt, hierduch ist der Ausdruck als dynamischer Text für das System kenntlich. Der Maßstab wird nicht sofort angepasst aber bei weiteren Aktivitäten wie z.B. beim Drucken. 
