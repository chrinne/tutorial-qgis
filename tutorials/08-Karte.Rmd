...
- closos-navetas.shp: nach @freyNavetasEsClosos1966a, Abb. 7, georeferenziert anhand der Luftbilder und in Umrissen digitalisiert,
...
Die Position der Schrift in der Mitte der Naveta stört mit sehr, dies können wir im Unterpunkt *Platzierung* ändern. Ich wechsele von "Um Zentrum" zu "Abstand vom Zentrum". Die Zuweisung der Position nach Quadranten um das Zentrum reicht nicht, deshalb wähle ich den "X, Y Versatz". Eine feste Größe von 12 m  für y wird den sehr unterschiedlichen Größen der Navetas nicht gerecht. Aus diesem Grund wähle ich auch hier aus dem *drop-down* am rechten Rand "Bearbeite" und komme so zum **Ausdruckseditor**.

```
concat('0,',bounds_height($geometry)*0.8)
```

Das erwartete Format ist ein Vektor mit zwei, durch ein Komma getrennten Distanzwerten für x und y (x.x, y.y). Ich starte mit dem zweiten, dem y-Wert: Dieser soll abhängig von der Geometriehöhe sein (*bounds_height(\$geometry)*), die volle Höhe (y-Achse) ist aber zuviel, deshalb  wähle ich 80% der Höhe. Eine Verschiebung auf x ist nicht nötig, also '0, ' und beide Ausdrücke werden mit *concat()* zu einer Zeichenkette verbunden.

### Elemente der Karte

Die Gestaltung des Layout erfolgt in einem neuen Fenster mit umfangreichen Icon-Leisten (abhängig von den Einstellungen unter "Ansicht"), einem leeren Blatt im größten Teil des Fensters und den Eigenschaften des aktiven Elementes am rechten Rand. Um die Papiergröße und -ausrichtung nachträglich zu verändern müssen Sie 1. im Kontextmenü (rechte Mausklick) zum Papier die **Seiteneigenschaften** wählen, die Sie dann 2. rechts auf dem Reiter zu den Elementeigenschaften anpassen können. Elemente im Layout werden rechts oben unter dem Register **Elemente** aufgeführt und können hier u.a. umbenannt werden. Dies ist bei vielen Elementen und Detailkarten sehr hilfreich. Dort finden Sie auch den Reiter Rücknahmeprotokoll mit einer Liste der Arbeitsschritte.

Wechseln Sie unterhalb rechts auf den Reiter **Führungen** und ergänzen Sie als erstes horizontale Führungen bei 20 mm, 160 mm und 190 mm sowie vertikale Führungen bei 20 mm und 277 mm.

![Closos de Can Gaià. Stukturen nach Frey y Reslló (1966, Abb. 7) und Vegetationsdichte nach Luftbildern (Kartengrundlage: IDEIB, EPSG: 25831)](./images/08_Closos_Vegetation.jpg)
...
Die **Legendenelemente** werden *a priori* automatisch verwaltet und entsprechen den aktiven Layern des Projektes. Deaktivieren Sie diese Option und Sie können nachfolgend die Einträge editieren, entfernen oder ergänzen. Hier gibt es für Gruppen, Layernamen und Symbolbezeichnung Gestaltungsoptionen. In unserem Fall sieht der doppelte Eintrag zu der ermittelten Vegetation unschön aus. Editieren Sie den ersten Legendeneinrag zu "Vegetation der Orthofotos", ändern den zugehörigen Unterpunkt von "1" zu "Orthofoto 1956". Für den zweiten Legendeneintrag öffnen Sie das Kontextmenü und wählen dort "verstecken", den zugehörigen Unterpunkt ändern Sie analog zum Vorangehenden von "1" zu "Orthofoto 2018". Alternativ erstellen Sie in Ihrem Projekt eine Gruppe "Vegetation Orthofoto" und verstecken für beide Legendeneinträge.  Ergänzen Sie noch die Werte für die ermittelten Vegetationsflächen bei dem jeweiligen Layer (1956: 17500 m², 2018: 57536 m²). 

Verändern Sie selbständig den Legendeneintrag für die Navetas und löschen Sie alle weiteren Legendeneinträge. Ändern Sie noch Schriftarten: alles auf 10 pt, Spalten: 2, Symbol: Größe 5 mm x 3 mm, Zwischenräume: Spaltenabstand auf 6 mm. Ergänzen Sie bei den Haupteigenschaften der Legende noch unter Titel: Legende. 

### Grafische Elemente

Fügen Sie einen **Maßstab** hinzu. Wählen Sie "Rechts 5" Segmente und Feste Breite: 10,0 Einheiten, dadurch erhalten am Anfang und Ende einen schwarzen Balken und die Breite entspricht auch der Unterteilung des Rahmens. Setzen Sie die Höhe auf 1,0 mm und passen Sie unter Anzeige ebenfalls den Beschriftungsrand (1 mm) und die Textgröße an den Stil der Legende oder des Rahmens an.
...
### Export
Die Karte kann als Ratsergrafik in diversen Formaten, als SVG (Scalable Vector Graphic) oder PDF (Portable Document File) gespeichert werden. Jedes Format hat Vor- und Nachteile. SVG ist a priori Vektor und bindet nur die Rasterelemente, z.B. die Luftbilder, als Raster ein. PDF ist eine Kombination aus Vektor- und Rasterelementen wobei ursprüngliche Linien durchaus auch in einem  Raster resultieren. Dies passiert u.a. bei transparenten Layern. Beim Export werden WMS-Dienste für den Druck abgefragt, da die Datenmenge der Abfrage bisweilen limitiert ist und diese sich aus der Auflösung (pixel) und Fläche definiert, können hierbei Probleme entstehen. 
