...
- closos-navetas: shp-Datei mit den Baustrukturen des Fundplatzes.
...
Wählen Sie nun "Bearbeiten -> Punkt hinzufügen" (\<strg> + \<a>) und markieren Sie nacheinander einen Punkt im Raster und dann den korrespondierenden Referenzpunkt in der Karte. Die Referenzpunkte in der Karte können in einem neuen Fenster wahlweise von Hand eingetragen oder mit [Aus Kartenansicht] in der Karte markiert werden. Wiederholen Sie den Vorgang für die vier Ecken, nutzen Sie beim Raster das Scrollrad der Maus zum Zoomen. Rote Linien zeigen die projektive Verschiebung der Punkte, dies kann ein Hinweis auf mögliche Fehler sein, irritiert bisweilen aber auch nur. Die additiv geführte Punktliste gibt berechnete Fehlerwerte aus und erlaubt das aktivieren oder deaktivieren einzelner Punkt. Mit dem Icon "grüner Pfeil / Play" wird die Transformation ausgeführt und der Vorgang beendet. 
...
Vor allem im Feld 1 sind einige rundliche Anomalien eher positiver (dunkler) Messwerte zu erkennen. Markieren Sie bei den Layern das Messfeld 1 und prüfen Sie die Messwerte mit der Objektabfrage (?-Icon) aus der Menüleiste. Die Messwerte liegen überwiegend zwischen 7 und 16 nT nur bei der Anomalie mit weißem Rand werden auch über 50 nT erreicht. 
...
Fügen Sie mit einem Doppelklick das Band 1 des Messfeldes 1 zum Ausdruck hinzu (closos_1_modifiziert\@1). Tragen Sie oben rechts den Namen für die Ausgabedatei ein, z.B. closos_1_7-16nT, nutzen Sie aber die [...] um auch den Ordner zu wählen. Für die Ausdehnung dieser Datei klicken Sie auf [Gewählte Layerausdehnung] und das entsprechende KBS der Ursprungsdatei oder des Projektes. 
...
| Anmerkung |
|----|
|Wenn **Fehler** auftreten prüfen Sie bitte als erstes die Layernamen (*copy & paste*) und die gewählte Dimension des Ausgaberasters. |

### Luftbilder und Vegetation

Augenscheinlich hat die Vegetation im Bereich der Fundstelle seit 1956 deutlich zugenommen. Das ist ein "alter Hut" und hängt mit den fehlenden Schaf- und Ziegenherden zusammen. Ich möchte diese Erkenntnis  nachvollziehbar quantifizieren. Wir müssen also bei dem Orthofoto von 1956 die sehr dunklen bis schwarzen Pixel zählen (Schwellwert sei <50) und hieraus die Fläche ableiten. Bei dem Farbfoto die eher grünen Flächen (RGB = Rot, Grün, Blau!). Der erste Fall ist nach der vorangehenden Übung trivial, lösen Sie den bitte eigenständig. Der zweite Fall braucht etwas Denksport und einfache Farbenlehre. 

Ein erster schneller Ansatz wäre dann, nur den Grünen Kanal auszuwerten: viel Grün (z.B. 0 - 75) entspricht viel Vegetation. Das stimmt aber nicht, denn wir bekommen sehr viele verschatteten (schwarzen) Bereiche mit rein, in denen alle drei Farbkanäle annähernd gleiche und hohe Werte besitzen. Dagegen verlieren wir die hellgrünen Bereiche. Die Aufgabe ist also nicht so trivial.  

Falls Sie mein erster schneller Ansatz interessiert:
...
("orthoaktuell@1" < ("orthoaktuell@2"*0.8)))
```
Nach einer kurzen Überprüfung der Werte liegen Grün und Blau annähernd paritätisch zwischen 50 und 110 während Rot stets etwas niedriger liegt bis maximal 80% des Blau- oder Grünwertes. Im Ergebnis stehen also über und unter dem Bruch identische Ausdrücke mit jeweils drei verknüpften Bedingungen. Das Ergebnis ist sicher nicht optimal, aber besser als die einfache Auswertung des grünen Kanals.

In der Information zum Layer finden Sie die Pixelgröße: rund 0.2000 (m) * 0.2000 (m). In der Toolbox können Sie mit "Rasterlayerstatistik" die Summe der Pixel abfragen und erhalten damit die Anzahl der gültigen Pixel da in diesen zelle ja "1" steht. Aus dem Produkt aller Werte ergibt sich die jeweils ermittelte Fläche an Vegetation. Alternativ benutzen Sie "Bericht eindeutiger Rasterwerte" und erhalten direkt auch die Fläche.
