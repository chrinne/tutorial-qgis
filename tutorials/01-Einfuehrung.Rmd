...
Dieses Tutorial ist für Archäologen gedacht und will einen Einstieg in die einzelnen Aspekte der Arbeit mit und in die Bereiche eines geographischen Informationssystems bieten. Dabei werden immer wieder archäologische Aspekte und Fragen mit Raumbezug aufgegriffen, um damit zugleich auf die Entwicklung und Bedeutung im Fach hinzuweisen. Dies erfolgt auf der Basis von Daten für die Baleareninsel Mallorca, eine Insel mit großer kulturellen Vielfalt und reichem Denkmalbestand. 
...
2. Allgemeines zu Datentypen, die Datenhaltung und Organisation von kleineren Projekten als auch der Einstieg in QGIS werden hier behandelt.    
...
Die verwendete **QGIS-Version 3.165 LTR** und kann bei [https://qgis.org](https://qgis.org) heruntergeladen werden.
...
Es gab und gibt aber auch deutliche Unterschiede zu den genannten IT-Bereichen. So haben GIS (pl.) im Desktop-Bereich über viele Jahre mit einzelnen Dateien (\*.shp) als Datenquelle gearbeitet und mit jedem Arbeitsschritt eine neue Datei produziert. Ein absolutes *no go* für ein Datenbanksystem. GIS [pl.] entdecken erst langsam den 2.5D oder echten 3D-Bereich, sie sind nicht wirklich mit CAD-Programmen vergleichbar. Die statistischen Optionen stehen den Möglichkeiten in R oder anderen Softwarepaketen deutlich nach. Und ein Grafikprogramm soll QGIS mit Sicherheit auch nicht sein. Dennoch, ein GIS ist für die meisten Aufgaben in der Archäologie ein unverzichtbares und oft einfach zu nutzendes Werkzeug. Und im Vergleich zu den Bereichen "Schreiben" und "Grafik" haben wir relativ viele gute freie Software zu Auswahl.

QGIS bietet eine Menge an Dokumentation, Einführungen und Hilfe [QGIS-Dokumentation](https://qgis.org/de/docs/index.html). Die hier aufgeführten Seiten und Dokumente sind aber nicht immer vollständig übersetzt, Sie kommen um Englisch nicht drumherum. Diese Einführung ist kein Ersatz für das umfangreiche [Handbuch zu QGIS im Internet](https://docs.qgis.org/3.22/de/docs/user_manual/). Erläuterungen u.a. zu den diversen Werkzeug- bzw. Icon-Leisten finden Sie dort unter "[GUI](https://docs.qgis.org/3.22/de/docs/user_manual/introduction/qgis_gui.html)".  Wenn Sie sich auf der Website von QGIS etwas umsehen erkennen Sie die sehr große Nutzergemeinschaft und damit einen großen Fundus an Kompetenz und Hilfestellung im Internet.   
...
- [gvSIG](http://www.gvsig.org): Ursprünglich aus Spanien stammend (Sistema Informativo Geográphico) zu dem es noch ein Derivat gibt [gvSIG OAdigital](http://oadigital.net)  
...
Ich wähle den Download, nicht die Netzwerkinstallation, und hier die langfristige Version (LTR), die meist stabiler läuft.

Die Installation ist einfach: Bestätigen Sie die üblichen Vorgaben und bei *Komponenten auswählen* die Vorgabe ohne Daten. 
...
![Karte von Menorca im Museum von Maó mit vier Koordinatensystemen: Längengrad bezogen auf Paris, Teneriffa, Cadiz und Karthago](./images/00_MenorcaMeridian.JPG)
