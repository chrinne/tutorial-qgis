...
- KBS: EPSG 25831 (bei der Gelegenheit ändern wir die Projektion auf die Vorgabe unseres Projektes), 
- FID: fid (feature id), 
- Geometry_Name: geom. 

Die **Nomenklatur für die *feature id* (fid) und vor allem die Geometrie (geom) sollte in der Datenbank einheitlich, kurz und aussagekräftig sein**, das erleichtert die spätere Arbeit ungemein. Wiederholen Sie den Vorgang für die Provinzen (Layername: provincias), Gemeinden (Layername: municipios) und die selbst generierten Dateien comarcas der Landschaftszonen (Layername: comarcas), ändern Sie in der Tabelle der Felder den Namen der Spalte für die  Comarcas auf "comarca" und abschließend exportieren sie noch mallorca-sites (Layername: sites). Exportieren Sie auch die Datentabelle pop-balears in die GPKG, das "-" im Namen wird automatisch entfernt, fügen Sie dieses aus didaktischen Gründen bitte wieder ein und wählen Sie "Ohne Geometrie".

Exportieren Sie zuletzt die Rasterdatei des Höhenmodelles mdt200 bzw. DGM200 in das Geopackage.  Mit dem GeoTIFF klappt dies problemlos, mit der früheren asc-Datei folgte eine Fehlermeldung:
...
- ```select municipio, 1950, '2011' from 'pop-balears' limit 3;```
...
| Achtung |
|:----|
| Das Entfernen des "-" im Layernamen durch das System, eigentlich der Tabellenname, war sehr weise, es macht die '' um den Namen überflüssig. Spaltennamen mit einer Ziffer am Anfang sind fast immer ein Problem und müssen vermieden werden. |
</br>

Mit der folgenden Abfrage erhalten wir den Namen und die Fläche der naturräumlichen Gliederung (*comarcas*). Die Syntax muss nicht wirklich erklärt werden. Mit wenigen Schritten bzw. Anweisungen könnten wir hier auch gleich die Talaiots zählen und diese Anweisung als *view* speichern.
...
select comarca as "Naturraum", round(st_area(geom)/100000,0) as "Fläche km²" 
from comarcas;
```
...
Dies sind allgemein gültige Regeln, nicht nur für die GPKG in QGIS. Wechseln wir nun aber zu SpatiaLite und zur SpatiaLite-GUI.
...
1. Wählen Sie hier bei "**latest version supporting spatialite GUI**", je nach System 32bit oder 64bit. Nur bei dieser Version ist die GUI dabei.
...
2. Fügen Sie diese Anweisung oberhalb im Abfragefeld ein, ändern Sie die erste Zeile zu: ```CREATE TABLE 'talaiots_municipios_tbl' AS``` und führen Sie alles aus.
...
| Rückblick Chi²-Test und eleganterer Weg ohne *join* |
|:----|
| Sie entsinnen sich des Tests zur Verteilung der Talaiots in den *comarcas* und der Tabelle mit zugehörigen Flächen? Wie das Vorangegangene erstellt die folgende Syntax  diese Tabelle in einem Rutsch und gleich mit schönen Spaltennamen.|
| Nehmen Sie sich etwas Zeit es in ruhe zu lesen, Sie sollten es eigentlich verstehen. Neu ist nur *group by* 1, womit verkürzt auf die erste Spalte der Abfrage verwiesen wird (m_comarca). |
</br>
